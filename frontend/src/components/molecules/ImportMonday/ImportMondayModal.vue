...
// Monday.com column synonyms (mirrors mondayRules CANDIDATES on the server).
const CANDIDATES = {
...
function parseFile(event) {
    ...
    reader.onload = (loadEvent) => {
        try {
            ...
            console.error('ERROR parsing Monday import file: ', error);
...
function downloadSample() {
    const csv = [
        'Name,Status,Priority,Due Date,Notes',
        'Design login screen,To Do,High,2026-06-25,Create the Figma mockups for the login flow',
        'Implement auth API,Working on it,High,2026-06-28,JWT-based login plus refresh tokens',
        'Write unit tests,To Do,Medium,2026-07-02,Cover the auth controller and helpers',
    ].join('\n');
    triggerDownload('alianhub-monday-sample.csv', csv, 'text/csv;charset=utf-8;');
}
...
function startImport() {
    ...
    apiRequest('post', '/api/v2/imports/monday', {
        rows: rows.value,
        ...
        console.error('ERROR in monday import: ', error);
        $toast.error(t('Toast.something_went_wrong'), { position: 'top-right' });
    }).finally(() => {
