...
<div id="project-step-container" class="ProjectShareGraphicModel addProjectEnabale borderChange mobile-project-taskstatus-section project-detail-setup">
    ...
    >
        <h3 :class="{'task-heading-desktop': clientWidth > 767 , 'task-heading-mobile': clientWidth <= 767}">{{ $t('ProjectDetails.details_project_setup') }}</h3>
    </div>
    <div class="Allgood">
        <div class="d-flex justify-content-between align-items-center allgoodproject allgoodprojectvalue">
            <h5 :class="{'font-size-13 font-weight-500': clientWidth > 767 , 'font-size-16 font-weight-400': clientWidth <= 767}">{{ $t('Milestone.project_name') }}</h5>
            <h6>{{theModel.projectName.value}}</h6>
        </div>
        <div class="d-flex justify-content-between align-items-center allgoodproject allgoodprojectvalue">
            <h5 :class="{'font-size-13 font-weight-500': clientWidth > 767 , 'font-size-16 font-weight-400': clientWidth <= 767}">{{ $t('Milestone.avatar') }}</h5>
        ...
        <div class="d-flex justify-content-between align-items-center allgoodproject allgoodprojectvalue">
            <h5 :class="{'font-size-13 font-weight-500': clientWidth > 767 , 'font-size-16 font-weight-400': clientWidth <= 767}">{{ $t('ProjectDetails.shared_with') }}</h5>
            <div class="d-flex align-items-center" v-if="theModel.workSpaceField.privateSpaceValue.value">
                <h6>{{ $t('UserTimesheet.users') }}</h6>
            ...
            <div class="d-flex align-items-center font-size-13 GunPowder" v-else>
                {{ $t('ProjectDetails.no_user_selected') }}
            </div>
        </div>
        <div class="d-flex justify-content-between align-items-center allgoodproject allgoodprojectvalue">
            <h5 :class="{'font-size-13 font-weight-500': clientWidth > 767 , 'font-size-16 font-weight-400': clientWidth <= 767}">{{ $t('Templates.project_status') }}</h5>
        ...
        <div class="d-flex justify-content-between align-items-center allgoodproject allgoodprojectvalue">
            <h5 :class="{'font-size-13 font-weight-500': clientWidth > 767 , 'font-size-16 font-weight-400': clientWidth <= 767}">{{$t('Templates.task_status')}}</h5>
        ...
        <div class="d-flex justify-content-between align-items-center allgoodproject allgoodprojectvalue">
            <h5 :class="{'font-size-13 font-weight-500': clientWidth > 767 , 'font-size-16 font-weight-400': clientWidth <= 767}">{{$t('Templates.apps')}}</h5>
            <ul class="d-flex align-items-center" v-if="!alianAppsFlag">
                <template v-for="(appData,ids) in theModel.alianAppsObj">
                    <li class="setImagevalue" v-bind:key="ids" v-if="appData.appStatus">
                        <span :title="$t(`Apps.${appData.key}`)">
                            <img v-if="appData.afterIcon" :src="getImageUrl(appData)" style="height: auto;"/>
                        </span>
                    </li>
                </template>
            </ul>
            <span class="font-size-13 GunPowder" v-if="alianAppsFlag">{{$t('Templates.no_app_selected')}}</span>
        </div>
        <div class="d-flex justify-content-between align-items-center allgoodproject">
            <h5 :class="{'font-size-13 font-weight-500': clientWidth > 767 , 'font-size-16 font-weight-400': clientWidth <= 767}">{{$t('Templates.default_views')}}</h5>
            <ul class="d-flex align-items-center">
                <template v-for="(displayView,ids) in theModel.requiredViewobj.value">
                    <li class="setImagevalue" :key="ids" v-if="displayView.viewStatus">
                        <span :title="$t(`ViewList.${displayView.name}`)">
                            <img :src="!displayView.viewStatus ? projectComponentsIcons(displayView.keyName).icon : projectComponentsIcons(displayView.keyName).activeIcon" :alt="$t(`ViewList.${displayView.name}`)">
                        </span>
                    </li>
                </template>
            </ul>
